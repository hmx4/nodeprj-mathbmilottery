...
router.post('/lottery/:type/:times', function(req, res){
	res.setHeader('Access-Control-Allow-Origin', '*'); // 允許其他網站的網頁存取此服務
	var type = Number(req.params.type);    //從路由資訊中取出樂透種類type之值(type==0:大樂透；type==1:威力彩)
	var	times = Number(req.params.times);  //從路由資訊中取出號碼組數times之值
	
	// 假如type不是數字或不是0或1，則設定警示字串給value
	if(Number.isNaN(type) || (type!=0 && type !=1))
		value = "種類代碼不是數字或(不是0也不是1)";
	// 若op不是數值，則設定回傳提醒訊息(JSON格式)給value
	else if(Number.isNaN(times) || times<1 || times>100000)
		value = "組數不是數字或小於1或大於10萬";
	else
	{    // 通過種類代碼及組數符合格式的檢驗後，才開始進行計算
		try
		{
			// 引用相同目錄(routes)下的lottery.js
			lottery = require("./lottery.js");
			// 呼叫genLotteryNum方法(輸入為種類type及組數times)，以取得樂透號碼格式化後之字串
			value = lottery.genLotteryNum(type, times); 
		}
		catch(ex)
		{
			value = "產生樂透號碼時發生例外，原因如下：" + ex.toString();
		}
	}
	// 設定回傳結果之編碼為utf-8，網頁端才能正常顯示中文
	res.set(
